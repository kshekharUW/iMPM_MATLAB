function [force]=getForce(mu,Nn,Ne,icon,lx,ly,vnew,mv)
    ...
    for i=1:Ne
        ...
        for j=1:4 % loop over 4 gauss points
            ss=gp(1,j);tt=gp(2,j);
            gradN = getGrad([ss,tt],lx,ly);
            
            gv(1,1)=gradN(1,:)*vnew(iv);   % velocity gradient 
            gv(1,2)=gradN(2,:)*vnew(iv);   % velocity gradient 
            gv(2,2)=gradN(2,:)*vnew(iv+Nn);% velocity gradient 
            gv(2,1)=gradN(1,:)*vnew(iv+Nn);% velocity gradient 
            ...
            tau      = 2*mu*(ddev); 
            fcell      = fcell + (tau*gradN*lx*ly/4)';   
        end
        force(iv)   = force(iv)   - fcell(:,1);
        force(iv+Nn)= force(iv+Nn)- fcell(:,2);
    end